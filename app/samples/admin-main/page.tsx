...
import { SAMPLE_ADMIN } from '@/lib/samples/mock-data';
...
export default function SampleAdminMainPage() {
    ...
    <SubPageFrame title="관리자" showBackButton={false} showMyPage={false}>
      ...
      {MENU_SECTIONS.map((section) => (
        <div key={section.id} className="mb-4">
          ...
          <div style={CARD}>
            {section.items.map(({ id, label, desc, icon: Icon, color, bg }, idx) => (
              <div key={id}>
                {idx > 0 && <div style={OHGO_LIST_DIVIDER} />}
                <button type="button" className="btn ohgo-menu-list-row" onClick={() => undefined}>
      ...
      <button
        type="button"
        onClick={() => router.push('/samples/main')}
